...
# duoSendAuth - Added by Lee Berg 
# Requires AUTH API enabled on Duo Instance
<# 
    .Synopsis
     Used to push to the Auth API
     Requires an AUTH DUO dorg 

    .Example
     duoSendAuth -dOrg 'AUTH' -username 'jsmith' -device 'deviceID-12345' -PromptTitle 'Warning" -PushInfo 'Message=Hello World'

#>

function duoSendAuth()
{
    param
    (
        [parameter(Mandatory=$false)]
    ...
    $param =@{username="$username";factor="push";device=$device;type=$PromptTitle;pushinfo=$PushInfo}
        
    try
    {
        $request = _duoBuildCall -method $method -dOrg $dOrg -path $path -parameters $param 
    }
    catch
    {
        throw $_
